...
Le chapitre suivant, \textbf{«\,Préparation du Projet\,»}, formalise les besoins, la méthodologie de développement \textbf{en spirale} par cycles successifs et les environnements techniques retenus pour mener à bien la réalisation de KODA.
	...
	\textbf{Basic LLM Chain} du Block~3 (classification sémantique). OpenRouter n'est pas
	un LLM en soi : c'est la \textbf{couche d'accès} au LLM choisi. \textbf{SerpAPI}, lui,
	intervient uniquement au \textbf{Block~1} (branche \texttt{musique}) pour la recherche
	YouTube via Google, et ne doit pas être confondu avec cette couche d'inférence.
	...
	Les workflows n8n s'appuient sur PostgreSQL pour relier utilisateurs, configuration (\texttt{seeting}, clé primaire \texttt{idkoda} référencée par \texttt{utilisateur.idkoda}) et données de conversation ou d'accompagnement. La figure~\ref{fig:koda-chapitre-classes-schema} synthétise le modèle conceptuel de la \textbf{mémoire et de la persistance} dans le Cycle~1.
...
%----------------------------------------------------------------
	...
	\textbf{6}). Le \textbf{Block~6}, en fin de chaîne, regroupe l'extraction
	conditionnelle des \textit{notes} et la \textbf{mise à jour de la mémoire}
	persistante. Chaque bloc accomplit une tâche précise avant de transmettre ses
	résultats au suivant. L'ensemble du flux est orchestré par n8n.
  ...
  génération de la réponse contextualisée, puis \textbf{Block~6} (extraction des
  \textit{notes} si besoin, puis persistance et clôture du webhook). Sans cette
  ...
  \begin{itemize}[leftmargin=*, itemsep=0.55em]
  	...
  	\item \textbf{Block~6 — Notes et mémoire persistante.} En deux temps~: (1)~lorsque le scénario l'exige, extraction structurée des \textit{notes} (rappels, rendez-vous, alarmes)~; (2)~enregistrement des conversations, informations personnelles, historique récent (cinq derniers messages) et envoi de la réponse finale. Ce bloc matérialise le lien avec le schéma de données (utilisateur, \texttt{seeting} via \texttt{idkoda}, historiques, etc.) présenté en figure~\ref{fig:koda-chapitre-classes-schema}.
  \end{itemize}
  
  La figure~\ref{fig:webhook1-sequence-blocs} résume l'enchaînement général des
  six blocs du Webhook~1 (embranchements optionnels pour la présentation et les
  \textit{notes}, puis persistance dans le Block~6).
  
  \IfFileExists{public/dsg.png}{%
  	\begin{center}
  		\includegraphics[width=0.98\textwidth]{public/dsg.png}
  		\captionof{figure}{Webhook~1 --- diagramme de séquence général entre les six blocs (vue macro ; notes et persistance~: Block~6).}
  ...
  \IfFileExists{public/serpapi.png}{%
  	\begin{figure}[htbp]
  		...
  		\caption{Block~1 — intégration SerpAPI dans le workflow n8n (branche musique du \textit{Switch})}
  		\label{fig:block1-serpapi}
  	\end{figure}
  }{}
  \IfFileExists{public/block1-serpapi-search-music.png}{%
  	\begin{figure}[htbp]
  		\centering
  		\includegraphics[width=0.98\textwidth]{public/block1-serpapi-search-music.png}
  		\caption{Block~1 — nœud \textit{Search Music} (SerpAPI, moteur YouTube) et exemple de sortie (\texttt{search\_query}, \texttt{youtube\_url})}
  		\label{fig:block1-serpapi-result}
  	\end{figure}
  }{}
  ...
  %--- block2
	...
	\vspace{0.5em}
	\subparagraph{OpenRouter dans le Block~3 — agrégation et suivi d'usage}
	
	Au \textbf{Block~3}, \textbf{OpenRouter} est le point d'accès unique aux modèles de
	langage~: il ne remplace pas SerpAPI (réservé au Block~1, branche musique), mais
	...
	\IfFileExists{public/openrouter-dashboard.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.92\textwidth]{public/openrouter-dashboard.png}
			\caption{Block~3 — tableau de bord OpenRouter (clé API workflow n8n, suivi des dépenses par modèle)}
			\label{fig:openrouter-dashboard}
		\end{figure}
	}{}
...
\clearpage
	...
	\paragraph{Block 6 : Notes, structuration et mise à jour de la mémoire}
	
	Le \textbf{Block~6} clôt le Webhook~1 en regroupant deux responsabilités
	complémentaires~: d'abord l'\textbf{extraction conditionnelle} des \textit{notes}
	(lorsque le Block~3 a classé la demande en \texttt{note}), puis la
	...
	la \textbf{première partie} du Block~6 prend le relais \textbf{avant} toute insertion en
	...
	\begin{enumerate}[leftmargin=*, itemsep=0.25em]
		...
		\texttt{format\_valide}) \textbf{avant} l'enregistrement définitif (partie~2 du Block~6).
	\end{enumerate}
	...
	\begin{center}
		...
		\captionof{figure}{Block 6 (partie~1) — Sous-workflow n8n~: test \texttt{questiontype = note}, \textit{Basic LLM Chain~2} et structuration}
	...
	et l'événement reformulé, prêt pour insertion dans \texttt{notes} (partie~2 du Block~6)~:
	...
	\vspace{0.3em}
	\noindent
	La première partie du Block~6 fait le lien entre la \textbf{classification sémantique}
	(Block~3, type \texttt{note}) et les champs structurés exploitables en base~:
	...
	La \textbf{deuxième partie} du Block~6 intervient après la génération de la réponse par le
	Block~5 (et, le cas échéant, après la structuration des champs \texttt{note} en
	...
	identifiées lors du traitement. Chaque branche du Block~5 (mémorielle ou générale)
	...
	\begin{center}
		...
		\captionof{figure}{Block 6 (partie~2) — Diagramme de classes (persistance mémoire)}
		\label{fig:block6-dcb6}
	\end{center}
	\vspace{1em}
	
	%----------------------------------------------------------------
	\subsubsection{Synthèse visuelle du pipeline}
	
	La figure~\ref{fig:webhook1-sequence-blocs} (déjà présentée en début de section)
	résume l'enchaînement macroscopique des six blocs. Pour une vue \textbf{complète}
	du Webhook~1 (toutes les branches conditionnelles sur une seule figure), il est
	recommandé d'exporter le diagramme PlantUML
	\texttt{plantuml/koda-webhook1-blocks1-6-sequence.puml} vers
	\texttt{public/sequence-webhook1-complet.png} si une version plus détaillée que
	\texttt{dsg.png} est souhaitée en fin de chapitre.
	
	\IfFileExists{public/sequence-webhook1-complet.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/sequence-webhook1-complet.png}
			\caption{Webhook~1 --- diagramme de séquence détaillé (pipeline complet, blocs~1 à~6)}
			\label{fig:webhook1-sequence-complet}
		\end{figure}
	}{%
		\IfFileExists{public/captureworkflowinput.png}{%
			\begin{figure}[htbp]
				...
				\caption{Webhook~1 --- vue d'ensemble du workflow n8n (entrée et enchaînement des blocs)}
				\label{fig:webhook1-vue-ensemble}
			\end{figure}
		}{}}
	...
	n8n. Conçu en \textbf{six blocs} successifs, ce flux traduit dans l'ingénierie la
	théorie du double traitement évoquée en début de chapitre~: un traitement
	\textbf{rapide} lorsque l'historique récent suffit (Block~2), puis un traitement
	\textbf{plus approfondi} lorsque la question exige classification, contexte et
	formulation personnalisée (blocs~3 à~5).
	
	Le \textbf{Block~1} assure la \textbf{sécurité et l'action immédiate} du compagnon
	(fermeture, mot de passe, musique, déplacement) avant tout appel coûteux au modèle
	de langage. Le \textbf{Block~2} limite la redondance en confrontant la demande aux
	\textbf{cinq derniers échanges}, ce qui améliore la réactivité perçue et réduit la
	consommation de ressources. Le \textbf{Block~3} structure ensuite le sens du message
	(\texttt{USE\_MEMORY} / \texttt{NO\_MEMORY}, six catégories) et prépare un routage
	fiable vers les branches adaptées. Le \textbf{Block~4} matérialise la dimension
	\og humaine \fg{} du projet~: enregistrement d'une nouvelle personne, accueil d'un
	interlocuteur \texttt{unkonu}, ou poursuite directe pour un profil déjà connu. Le
	\textbf{Block~5} concentre la \textbf{valeur conversationnelle}~: réponse enrichie
	ou générale selon la mémoire requise, dans le ton et l'identité de KODA. Enfin, le
	\textbf{Block~6} clôt le cycle en deux temps~: structuration conditionnelle des
	\textit{notes} (rappels, rendez-vous), puis \textbf{persistance} (conversations,
	informations personnelles, historique glissant, réponse HTTP) conforme au schéma de
	données du chapitre.
	...
	(proactivité). L'architecture modulaire par blocs a facilité l'itération sur les
	...
	\IfFileExists{public/webhook2.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/webhook2.png}
			\caption{Webhook~2 --- vue d'ensemble du workflow n8n (collecte PostgreSQL, structuration, AI Agent, insertion \texttt{accompagnement})}
			\label{fig:webhook2-workflow-n8n}
		\end{figure}
	}{%
		\begin{center}
			\fbox{\begin{minipage}{0.88\textwidth}\centering\vspace{1em}
				\textit{Ajouter \texttt{public/webhook2.png} --- capture du workflow n8n Webhook~2.}
				\vspace{1em}\end{minipage}}
		\end{center}
	}
	
	\subsubsection{Diagramme de séquence du Webhook 2}
	
	La figure~\ref{fig:webhook2-sequence} résume les interactions entre le
	\textbf{backend} (trigger), le workflow n8n, PostgreSQL, les nœuds de structuration et l'agent IA.
	La source PlantUML est \texttt{plantuml/koda-webhook2-synthese-sequence.puml}.
	
	\IfFileExists{public/koda_webhook2_synthese_sequence.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/koda_webhook2_synthese_sequence.png}
			\caption{Webhook~2 --- diagramme de séquence (synthèse journalière et persistance)}
			\label{fig:webhook2-sequence}
		\end{figure}
	}{%
		\begin{center}
			\fbox{\begin{minipage}{0.88\textwidth}\centering\vspace{1em}
				\textit{Exporter \texttt{plantuml/koda-webhook2-synthese-sequence.puml} vers \texttt{public/koda\_webhook2\_synthese\_sequence.png}.}
				\vspace{1em}\end{minipage}}
		\end{center}
	}
	...
	\IfFileExists{public/4conseil.png}{%
		\begin{figure}[htbp]
			...
			\caption{Webhook~2 --- exemple de sortie structurée (quatre dimensions de la fiche journalière)}
			\label{fig:webhook2-output-4dimensions}
		\end{figure}
	}{%
	...
	\IfFileExists{public/webhook2-schema-accompagnement.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.72\textwidth]{public/webhook2-schema-accompagnement.png}
			\caption{Webhook~2 --- schéma de persistance (\texttt{utilisateur} $\rightarrow$ \texttt{accompagnement}~: \texttt{interets}, \texttt{synthese}, \texttt{point\_cles}, \texttt{conseils})}
			\label{fig:webhook2-schema-accompagnement}
		\end{figure}
	}{}
	...
	(branche mémorielle du Block~5) et exploitées par le \textbf{Webhook~3} pour générer
	...
	Le Webhook~3 est activé lorsque le \textbf{backend} ou le module de vision du robot
	détecte une présence et transmet un identifiant (\texttt{username}, \texttt{acteur}).
	Le workflow n8n (figure~\ref{fig:webhook3-workflow-n8n}) enchaîne~:
	\textit{Identité7} (PostgreSQL) $\rightarrow$ nœud \textit{Code} $\rightarrow$
	\textit{If7}, qui oriente soit vers \textbf{AI Agent3} (personne connue), soit vers
	\textbf{Postgres15} + \textbf{Code11} + \textbf{AI Agent2} (cas nécessitant une
	introduction enrichie à partir des listes issues du Flux~2).
	
	\IfFileExists{public/webhook3.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/webhook3.png}
			\caption{Webhook~3 --- workflow n8n (\textit{If7}, branches AI Agent2 / AI Agent3, réponses webhook)}
			\label{fig:webhook3-workflow-n8n}
		\end{figure}
	}{}
	
	\subsubsection{Préparation des données (nœud Code11)}
	
	Le nœud \textbf{Code11} consolide, pour chaque exécution, les listes produites à
	partir des fiches \texttt{accompagnement} (Webhook~2)~:
	\begin{itemize}
		\item \texttt{ma\_liste\_des\_centre\_d\_interet} $\leftarrow$ centres d'intérêt ;
		\item \texttt{ma\_liste\_des\_profils} $\leftarrow$ profils comportementaux ;
		\item \texttt{ma\_liste\_des\_points\_de\_vue} $\leftarrow$ points de vue ;
		\item \texttt{ma\_liste\_des\_conseils} $\leftarrow$ conseils personnalisés ;
		\item \texttt{username} --- identifiant transmis par le backend ;
		\item \texttt{utilisateur} --- \texttt{acteur} si connu, sinon \textbf{\texttt{Inconnu}}.
	\end{itemize}
	
	La figure~\ref{fig:webhook3-code11-input} illustre cette étape dans l'interface n8n.
	
	\IfFileExists{public/webhook3-code11-input.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/webhook3-code11-input.png}
			\caption{Webhook~3 --- entrée du nœud \textit{Code11} (listes issues du Webhook~2, \texttt{username}, \texttt{utilisateur} ou \texttt{Inconnu})}
			\label{fig:webhook3-code11-input}
		\end{figure}
	}{}
	
	\subsubsection{Génération de l'introduction et sortie JSON}
	
	L'\textbf{AI Agent} (modèle Google Gemini via n8n) produit une réponse structurée
	exploitée par le robot et l'interface~:
	\begin{itemize}
		\item \texttt{parole\_du\_robot} --- texte d'introduction à prononcer (ton amical,
		référence au prénom si connu, reprise des thèmes mémorisés) ;
		\item \texttt{emotion\_visuelle} --- indication d'affichage (ex. \emph{Clin d'œil}).
	\end{itemize}
	
	Exemple pour un utilisateur reconnu (\texttt{Oussema})~: \emph{«\,Salut Oussema !
	T'inquiète, je t'ai pas zappé, même si tu testes ma mémoire H24. Alors, on se fait
	un petit défi pour voir qui a la meilleure aujourd'hui ?\,»} avec
	\texttt{emotion\_visuelle = Clin d'œil} (figure~\ref{fig:webhook3-output-parole}).
	
	\IfFileExists{public/webhook3-output-parole.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.92\textwidth]{public/webhook3-output-parole.png}
			\caption{Webhook~3 --- sortie JSON (\texttt{parole\_du\_robot}, \texttt{emotion\_visuelle})}
			\label{fig:webhook3-output-parole}
		\end{figure}
	}{}
	
	\subsubsection{Diagramme de séquence du Webhook 3}
	
	La figure~\ref{fig:webhook3-sequence} synthétise le flux~: détection backend,
	lecture mémoire long terme (Webhook~2), structuration \textit{Code11}, génération
	par l'\textbf{AI Agent} et réponse au robot.
	Source PlantUML~: \texttt{plantuml/koda-webhook3-spontaneite-sequence.puml}.
	
	\IfFileExists{public/koda_webhook3_spontaneite_sequence.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.98\textwidth]{public/koda_webhook3_spontaneite_sequence.png}
			\caption{Webhook~3 --- diagramme de séquence (introduction de parole et mémoire Flux~2)}
			\label{fig:webhook3-sequence}
		\end{figure}
	}{}
	...
	\IfFileExists{public/ducc1.png}{%
		\begin{figure}[htbp]
			...
			\caption{Cycle~1 --- diagramme de cas d'utilisation (trois webhooks et blocs associés)}
			\label{fig:ducc1-cycle1}
		\end{figure}
	}{}
	...
	théorie du double traitement, OpenRouter (Block~3), SerpAPI (Block~1, branche musique)
	et PostgreSQL pour la persistance.
	
	Le \textbf{Webhook~1} (six blocs) assure le traitement en temps réel~: garde-fous et
	routage (Block~1), cache historique (Block~2), classification (Block~3), identité
	(Block~4), réponse contextualisée (Block~5), puis notes et mémoire (Block~6).
